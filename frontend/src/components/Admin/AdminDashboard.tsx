...
import { Users, UserPlus, Edit2, Trash2, Shield, AlertCircle, CheckCircle, FolderPlus, Drill } from 'lucide-react';
import { userService, projectService, drillholeService, companyService } from '../../api/services';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<'users' | 'projects' | 'drillholes'>('users');
  ...
  const [editingDrillhole, setEditingDrillhole] = useState<DrillholeWithDetails | null>(null);
  ...
  const loadUsers = async () => {
    try {
      ...
      showMessage('error', error.response?.data?.detail || 'Failed to load users');
    }
  };
  ...
  const loadProjects = async () => {
    try {
      ...
      showMessage('error', error.response?.data?.detail || 'Failed to load projects');
    }
  };
  ...
  const loadDrillholes = async () => {
    try {
      ...
      showMessage('error', error.response?.data?.detail || 'Failed to load drillholes');
    }
  };
  ...
  const handleDeleteProject = async (id: number) => {
    ...
    try {
      ...
      showMessage('error', error.response?.data?.detail || 'Failed to delete project');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateDrillhole = async (drillholeData: any) => {
    try {
      ...
      await drillholeService.create({
        ...
        depth: drillholeData.depth ? parseFloat(drillholeData.depth) : undefined,
      });
      showMessage('success', 'Drillhole created successfully');
      loadDrillholes();
      setShowCreateModal(false);
    } catch (error: any) {
      showMessage('error', error.response?.data?.detail || 'Failed to create drillhole');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateDrillhole = async (id: number, drillholeData: any) => {
    try {
      ...
      showMessage('error', error.response?.data?.detail || 'Failed to update drillhole');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteDrillhole = async (id: number) => {
    ...
    try {
      ...
      showMessage('error', error.response?.data?.detail || 'Failed to delete drillhole');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage users, projects, and drillholes</p>
        </div>
      </div>
      ...
      {activeSection === 'drillholes' && (
        <>
          ...
          <div className="bg-white rounded-lg shadow overflow-hidden">
            <div className="p-6 border-b">
              <h3 className="text-lg font-bold text-gray-800">Drillhole Management</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Drillhole ID</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {drillholes.map((drillhole) => (
                    <tr key={drillhole.id} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900">{drillhole.drillhole_id}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{drillhole.project_name || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{drillhole.depth || '-'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                          drillhole.status === 'active' ? 'bg-green-100 text-green-800' :
                          drillhole.status === 'completed' ? 'bg-blue-100 text-blue-800' :
                          'bg-gray-100 text-gray-800'
                        }`}>
                          {drillhole.status}
                        </span>
      ...
      {editingProject && (
        <ProjectModal
          ...
          onSubmit={(projectData) => handleUpdateProject(editingProject.id, projectData)}
          loading={loading}
        />
      )}
      ...
      {editingDrillhole && (
        <DrillholeModal
          ...
          onSubmit={(drillholeData) => handleUpdateDrillhole(editingDrillhole.id, drillholeData)}
          loading={loading}
        />
      )}
    </div>
...
const UserModal: React.FC<UserModalProps> = ({ user, onClose, onSubmit, loading }) => {
...
const ProjectModal: React.FC<ProjectModalProps> = ({ project, companies, onClose, onSubmit, loading }) => {
  const [formData, setFormData] = useState({
...
const DrillholeModal: React.FC<DrillholeModalProps> = ({ drillhole, projects, onClose, onSubmit, loading }) => {
  const [formData, setFormData] = useState({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Drillhole ID</label>
            <input
              type="text"
              value={formData.drillhole_id}
              onChange={(e) => setFormData({ ...formData, drillhole_id: e.target.value })}
              className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Depth (m)</label>
            <input
              type="number"
              step="0.01"
