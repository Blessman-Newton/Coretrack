...
export const dispatchesAPI = {
  getAll: (params?: {
    ...
    if (params?.drillhole_id)
      queryParams.append("drillhole_id", params.drillhole_id.toString());
  ...
  processReturn: (
    ...
    apiRequest(`/dispatches/${id}/return`, {
      method: "POST",
      body: JSON.stringify(data),
    }),
...
export const reportsAPI = {
  ...
  getReconciliation: (params?: {
    ...
    return apiRequest<any>(
      `/reports/reconciliation${query ? `?${query}` : ""}`
    );
  },
  ...
  exportCSV: async (params?: {
    ...
    const response = await fetch(
      `${FULL_API_URL}/reports/export${query ? `?${query}` : ""}`,
      { headers }
    );
    ...
    a.href = url;
    a.download = `tray_inventory_report_${
